...
public class PoseIMURecorder {
    ...
    public static final int PHOTO_NUMBER = 8;
    ...
    private String[] default_file_names_ = {"gyro.txt", "acce.txt", "magnet.txt", "linacce.txt",
            "gravity.txt", "orientation.txt", "pose.txt", "step.txt", "photo.txt"};

    public PoseIMURecorder(String path, Context parent) {
        parent_ = parent;
    ...
    public void endPhotoFile() {
        try {
            file_writers_[SENSOR_COUNT - 1].flush();
            file_writers_[SENSOR_COUNT - 1].close();
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public Boolean addPhotoNumber(long timestamp, int value) {
        try {
            file_writers_[PHOTO_NUMBER].write(String.format(Locale.US, "%d %d\n", timestamp, value));
        } catch (IOException e) {
            e.printStackTrace();
            return false;
        }
